...
public class RankPvpSystemComboKill
{
	...
	private List<Integer> _victims = new ArrayList<>();
	...
	public static final int SAY_TYPE = Say2.ANNOUNCEMENT;	// local or global area are defined by sending methods.
	
	/**
	 * Add victim information into combo list (_victims).
	 * @param victimId
	 * @param killTime
	 * @return
	 */
	 ...
	 * @param victim 
	 */
	public void shoutComboKill(L2PcInstance killer, L2PcInstance victim) // TODO check and correct the local and global shouts.
	{
		...
		if(!RankPvpSystemConfig.COMBO_KILL_ALT_MESSAGES_ENABLED)
		{
			...
			else
			{
				...
				for(Entry<Integer, String> value : RankPvpSystemConfig.COMBO_KILL_GLOBAL_AREA_MESSAGES.entrySet())
				{
					last = value;
				}
	...
	public double getComboKillRankPointsRatio()
	{
		if(getComboLevel() > 0)
		{
			Map<Integer, Double> list = RankPvpSystemConfig.COMBO_KILL_RANK_POINTS_RATIO;
