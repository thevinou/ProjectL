 ...
 */
package net.sf.l2j.gameserver.masteriopack.imageconverter;
...
import net.sf.l2j.gameserver.masteriopack.rankpvpsystem.Rank;
import net.sf.l2j.gameserver.masteriopack.rankpvpsystem.RankPvpSystemConfig;
import net.sf.l2j.gameserver.masteriopack.rankpvpsystem.RankTable;
...
public class ServerSideImage
{
	...
	{
		if(RankPvpSystemConfig.SERVER_SIDE_IMAGES_ENABLED)
			load();
		else
	...
	public String getExpImageHtmlTag(L2PcInstance player, int expId, int width, int height)
	{
		...
		if(RankPvpSystemConfig.SERVER_SIDE_IMAGES_ENABLED)
		{
    		try
    		{
    			int id = 400000000 + expId + (RankPvpSystemConfig.IMAGE_PREFIX * 200);
	...
	public String getRankIconImageHtmlTag(L2PcInstance player, int rankId, int width, int height)
	{
		...
		if(RankPvpSystemConfig.SERVER_SIDE_IMAGES_ENABLED)
		{
    		try
    		{
    			int id = 500000000 + rankId + (RankPvpSystemConfig.IMAGE_PREFIX * 200);
	...
	public String getRankNameImageHtmlTag(L2PcInstance player, int rankId, int width, int height)
	{
		...
		if(RankPvpSystemConfig.SERVER_SIDE_IMAGES_ENABLED)
		{
    		try
    		{
    			int id = 600000000 + rankId + (RankPvpSystemConfig.IMAGE_PREFIX * 200);
